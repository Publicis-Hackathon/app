...
Two cooperating agents turn brand constraints into polished creative generated by the FIBO model. The **FiboAgent** is responsible for proposing and iteratively improving imagery, while the **BrandGuidelineAgent** evaluates palette and gaze compliance until both agents agree on a final asset (`backend/agentic/workflow.py:13`, `backend/agentic/graph_builder.py:7`). This repo pairs that workflow with a Lovable/Vite frontend for reviewing the iterations.
